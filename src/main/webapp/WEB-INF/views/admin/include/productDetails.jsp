    ...
    <script>
        $(function () {
            if($("#details_product_id").val() === ""){
                ...
                $("#btn_product_save").click(function () {
                    ...
                    if(product_isEnabled === undefined){
                        styleUtil.errorShow($("#text_productState_details_msg"),"Please select the status of the product！");
                        yn = false;
                    }
                    if(product_name === ""){
                ...
                $("input[name='radio_product_isEnabled']").each(function () {
                    if($(this).val() === product_isEnabled){
                        ...
                        if($(this).val() === "1"){
                            $("#text_productState_details_msg").text("Tip: Trading is not available when the product is out of sale").attr("title","Tip: Trading is not available when the product is out of sale").css("left",0).css("opacity","1");
                        }
                        return false;
                    }
                });
                ...
                $("#btn_product_save").click(function () {
                    ...
                    var yn = true;
                    if (product_isEnabled === undefined) {
                        styleUtil.errorShow($("#text_productState_details_msg"), "select the status of the product！");
                        yn = false;
                    }
                    if (product_name === "") {
                        styleUtil.basicErrorShow($("#lbl_product_name"));
            ...
            $(".details_picList").on("click","li:not(.details_picList_fileUpload)",function () {
                    ...
                    $("#btn-ok").unbind("click").click(function () {
                        $.ajax({
                            ...
                            success: function (data) {
                                $("#btn-ok").attr("disabled", false).text("Confirm");
                                if (data.success) {
                                    img.remove();
                                    fileUploadInput.css("display", "inline-block");
                                    $('#modalDiv').modal("hide");
                            ...
                            beforeSend: function () {
                                $("#btn-ok").attr("disabled", true).text("in operation...");
                            },
                            error: function () {
                ...
                $(".modal-body").text("Sure to delete the image？");
                $('#modalDiv').modal();
            });
            //改变产品状态时
            $('input:radio').click(function () {
                if($(this).val() === "1"){
                    styleUtil.errorShow($("#text_productState_details_msg"),"Tip: Trading is not available when the product is out of sale");
                } else {
            ...
            $("#select_product_category").change(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        listDiv.empty().append("<span class='details_title text_info'>property information</span>");
                        //显示产品属性数据
        ...
        function uploadImage(fileDom) {
            ...
            if (file === undefined || !imageType.test(file.type)) {
                ...
                $(".modal-body").text("Select an image！");
                $('#modalDiv').modal();
                return;
            }
            //判断大小
            if (file.size > 10240000) {
                ...
                $(".modal-body").text("the size of the image cannot exceed 1MB！");
                $('#modalDiv').modal();
                return;
            }
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    $(fileDom).attr("disabled", true).prev("span").text("image uploading...");
                },
                error: function () {
...
<div class="details_div_first">
    ...
    <div class="frm_div_last warn_height">
        <label class="frm_label text_info" id="lbl_product_category_id" for="select_product_category">product type</label>
        ...
        <label class="frm_label text_info" id="lbl_product_isEnabled_true" for="radio_product_isEnabled_true">able to buy</label>
        <input id="radio_product_isEnabled_false" name="radio_product_isEnabled" type="radio" value="1">
        <label class="frm_label text_info" id="lbl_product_isEnabled_false" for="radio_product_isEnabled_false">sale pause</label>
        <input id="radio_product_isEnabled_special" name="radio_product_isEnabled" type="radio" value="2">
        <label class="frm_label text_info" id="lbl_product_isEnabled_special" for="radio_product_isEnabled_special">in sale</label>
...
<div class="details_div">
    ...
    <div class="frm_div">
        <label class="frm_label text_info" id="lbl_product_id">product no.</label>
        <span class="details_value" id="span_product_id">product id</span>
